...
(function () {
  ...
  function initDecisionMatrix() {
    ...
    if (!caseSelect || !updateFreqSlider) return;
    ...
    function calculateDecision() {
      ...
      if (freqValEl) freqValEl.textContent = freq > 70 ? 'Muy Alta (Diaria/Semanal)' : freq > 30 ? 'Media (Mensual)' : 'Baja (Estática)';
      if (styleValEl) styleValEl.textContent = style > 70 ? 'Extrema (Sintaxis/Tono Rígido)' : style > 30 ? 'Moderada' : 'Estándar';
      if (budgetValEl) budgetValEl.textContent = budget > 70 ? 'Alto (Cluster GPUs)' : budget > 30 ? 'Medio (1x GPU Comercial)' : 'Bajo ($0 / API pura)';
      if (hallValEl) hallValEl.textContent = hall < 20 ? 'Cero Tolerancia (Factual Estricto)' : hall < 60 ? 'Baja / Media' : 'Alta (Creativo)';
  ...
  function initVramSimulator() {
    ...
    function calculateVram() {
      ...
      if (totalVram <= gpuVram * 0.85) {
        ...
        badgeEl.textContent = '⚠️ Límite Crítico (Riesgo de OOM)';
  ...
  function initHyperparameterPlanner() {
    ...
    function renderHyperparameters() {
      ...
      if (effBatchEl) effBatchEl.textContent = `${effectiveBatch} ejemplos / step`;
      ...
      if (timeEstEl) timeEstEl.textContent = `~${timeStr} (en 1x RTX 4090)`;
  ...
  function initLlmJudgeSimulator() {
    ...
    if (!caseSelect || !evaluateBtn) return;
    ...
    function runEvaluation() {
      const c = JUDGE_CASES[caseSelect.value];
      if (!c) return;
      ...
      // Render Rúbrica Juez
      if (scoreFactEl) scoreFactEl.textContent = '★'.repeat(c.scores.fact) + '☆'.repeat(5 - c.scores.fact);
      if (scoreRelEl) scoreRelEl.textContent = '★'.repeat(c.scores.rel) + '☆'.repeat(5 - c.scores.rel);
      if (scoreCohEl) scoreCohEl.textContent = '★'.repeat(c.scores.coh) + '☆'.repeat(5 - c.scores.coh);
      if (scoreFormatEl) scoreFormatEl.textContent = '★'.repeat(c.scores.format) + '☆'.repeat(5 - c.scores.format);
